 ...
 */
export async function searchWiki(paths: VaultPaths, query: string, maxResults = 5): Promise<RecallResult[]> {
  ...
  // If wiki returned < maxResults, search vault notes
  if (top.length < maxResults) {
    const notesDir = join(paths.root, "notes");
    if (existsSync(notesDir)) {
      ...
      const remaining = maxResults - top.length;

      const vaultScored: Array<{ id: string; score: number }> = [];
      for (const page of pages) {
        ...
        if (score > 0) vaultScored.push({ id: page.relative, score });
      }

      vaultScored.sort((a, b) => b.score - a.score);
      for (const vs of vaultScored.slice(0, remaining)) {
        if (!top.some((s: any) => s.id === vs.id)) {
          const page = pages.find((p: any) => p.relative === vs.id);
          if (page) {
            const fm = parseVaultFrontmatter(page.content);
            const preview = page.content.replace(/^---[\s\S]*?---\n/, "").trim().slice(0, 200).replace(/\n/g, " ");
            top.push({
              id: vs.id,
              entry: { title: String(fm.frontmatter.title || vs.id), type: String(fm.frontmatter.type || "note") } as Registry["pages"][string],
              score: vs.score,
            });
          }
        }
