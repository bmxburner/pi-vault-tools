...
export function registerWikiStatus(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "vault_status",
    label: "Vault Status",
    description: "Report vault health and stats: total notes, frontmatter completeness, orphans, and wiki overlay status.",
    promptSnippet: "Report vault health and stats",
    promptGuidelines: ["Use vault_status for a quick overview of the notes vault and wiki overlay."],
    parameters: Type.Object({}),
    async execute() {
      const root = resolveVaultRoot(process.cwd());
      const paths = getVaultPaths(root);

      // Wiki overlay stats (may be absent after archive)
      let wikiPages = 0;
      let wikiHealth = "Archived";
      let wikiLastUpdated = "N/A";
      if (existsSync(paths.dotWiki)) {
        ...
        wikiPages = Object.keys(registry.pages).length;
        wikiLastUpdated = registry.last_updated || "Never";
        wikiHealth = wikiPages === 0 ? "Empty" : "Active";
      }

      // Vault notes stats
      let vaultNoteCount = 0;
      let vaultWithFrontmatter = 0;
      let vaultOrphans = 0;
      const notesDir = join(root, "notes");
      if (existsSync(notesDir)) {
        const { findVaultPages, parseVaultFrontmatter } = await import("./utils.js");
        const pages = findVaultPages(notesDir);
        vaultNoteCount = pages.length;
        for (const page of pages) {
          const fm = parseVaultFrontmatter(page.content);
          if (fm.missingRequired.length === 0) vaultWithFrontmatter++;
          const links = page.content.match(/\[\[([^\]|]+)(?:\|[^\]]*)?\]\]/g);
          if (!links || links.length === 0) vaultOrphans++;
        }
      }
      ...
      const lines = [
        "📊 Vault Status",
        "═══════════════",
        `Wiki overlay: ${wikiHealth} (${wikiPages} pages, last: ${wikiLastUpdated})`,
        `Vault notes: ${vaultNoteCount}`,
        `  Complete frontmatter: ${vaultWithFrontmatter}/${vaultNoteCount}`,
        `  Potential orphans: ${vaultOrphans}`,
      ];

      return {
        content: [{ type: "text", text: lines.join("\n") }],
        details: {
          wiki: { pages: wikiPages, health: wikiHealth, lastUpdated: wikiLastUpdated },
          vault: {
            totalNotes: vaultNoteCount,
            withCompleteFrontmatter: vaultWithFrontmatter,
            potentialOrphans: vaultOrphans,
          },
        } as Record<string, unknown>,
      };
    },
        ...
        content: [
          {
            ...
            text: [
              ...
              "",
              "```",
              `schedule_prompt action=add schedule="${config.cron}" prompt="Run vault_lint for vault health check" name="vault-tools-autoupdate"`,
              "```",
              "",
